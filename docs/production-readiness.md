...
- [ ] Add a `LICENSE` (MIT suggested). Note in the README that `vendor/superpowers`
      and `vendor/spec-kit` retain their own upstream licenses and are not
      relicensed by this repo.
- [ ] README: add a **Requirements** section — git ≥ 2.13 (submodules), bash ≥ 4
      (associative arrays in `install.sh`), GNU coreutils on macOS *until Phase 1
      removes the dependency*.
- [ ] README: state the support posture honestly ("maintained by one person;
...
- [ ] **Portability fix in `install.sh`**: replace `declare -A` with parallel
...
- [ ] **`.github/workflows/ci.yml`** on every PR + master push:
...
- [ ] **Changelog gate**: CI check that PRs touch `CHANGELOG.md` (enforces the
...
- [ ] Document the release ritual in CLAUDE.md (move `[Unreleased]` → version
      ...
      from the changelog.
- [ ] README install snippet: pin the tag —
...
- [ ] `install.sh --verify` (or `--doctor`): report every expected link, whether
      it exists, resolves, and is damascus-owned — the first thing to ask for in
      any bug report.
- [ ] `install.sh --dry-run`: print planned actions without touching anything.
- [ ] **Stale-link pruning**: install currently only writes the *current* name
      ...
      current set.
- [ ] Friendlier failures for the known sharp edges: uninitialized submodules
...
- [ ] Extend the CI smoke test to cover `--verify`, `--dry-run`, and the prune
...
- [ ] **Dependabot for submodules** (`package-ecosystem: gitsubmodule`, monthly):
...
- [ ] Bump policy note in CLAUDE.md: only merge bumps that land on an upstream
      *tag*; re-check the DENY/KEEP/CONDITIONAL table when superpowers adds or
      ...
      DENY).
- [ ] Minimal `.github/ISSUE_TEMPLATE/bug.md` asking for `install.sh --verify`
